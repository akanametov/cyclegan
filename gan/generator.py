...
class EncoderBlock(nn.Module):
    """Encoder block"""
    def __init__(self, inplanes, outplanes, kernel_size=4, stride=2, padding=1, norm=True, padding_mode='zeros'):
        super().__init__()
        self.lrelu = nn.LeakyReLU(0.2, inplace=True)
        self.conv = nn.Conv2d(inplanes, outplanes, kernel_size, stride, padding, padding_mode=padding_mode)
...
class ResidualBlock(nn.Module):
    """Residual block"""
    def __init__(self, planes, kernel_size=3, stride=1, padding=1, norm=True, padding_mode='reflect'):
        super().__init__()
        self.encoder = EncoderBlock(planes, planes, kernel_size, stride, padding, norm, padding_mode)
        self.conv = nn.Sequential(
                nn.Conv2d(planes, planes, kernel_size, stride, padding, padding_mode=padding_mode),
...
class ResGenerator(nn.Module):
    """Encoder-Decoder model with Residual blocks"""
    def __init__(self, num_blocks=6):
        super().__init__() #, padding_mode='reflect'
        
        self.encoder1 = nn.Conv2d(3, 64, kernel_size=7, stride=1, padding=3, padding_mode='reflect')
        self.encoder2 = EncoderBlock(64, 128, 3, padding_mode='reflect')
        self.encoder3 = EncoderBlock(128, 256, 3, padding_mode='reflect')
        
        self.resblocks = self._make_resblock(256, num_blocks)

        self.decoder3 = DecoderBlock(256, 128)
        self.decoder2 = DecoderBlock(128, 64)
        self.decoder1 = nn.Conv2d(64, 3, kernel_size=7, stride=1, padding=3, padding_mode='reflect')
    ...
    def forward(self, x):
        ...
        e3 = self.encoder3(e2)
        # resblocks forward
        res = self.resblocks(e3)
        # decoder forward
        d3 = self.decoder3(res)
        d2 = F.relu(self.decoder2(d3))
        d1 = self.decoder1(d2)
...
class UnetResGenerator(nn.Module):
    ...
    def __init__(self, num_blocks=6):
        ...
        self.decoder3 = DecoderBlock(2*256, 128)
        self.decoder2 = DecoderBlock(2*128, 64)
        self.decoder1 = nn.Conv2d(2*64, 3, kernel_size=7, stride=1, padding=3, padding_mode='reflect')
